...
export function SettingsPage() {
  const { settings, updateSetting, saveToBackend, resetDefaults } = useSettings();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="space-y-6">
          <SettingsSection title="Monitoring" description="Data collection and refresh settings" icon={Gauge}>
            <SettingRow
            ...
            <SettingRow
              ...
              control={<Switch checked={settings.autoReconnect} onCheckedChange={(v) => updateSetting('autoReconnect', v)} />}
            />
          </SettingsSection>
          ...
          <SettingsSection title="Display" description="Visual preferences" icon={Eye}>
            <SettingRow
              ...
              control={<Switch checked={settings.showSensitiveData} onCheckedChange={(v) => updateSetting('showSensitiveData', v)} />}
            />
            <Separator />
            <SettingRow
              label="Compact mode"
              description="Reduce spacing in data tables"
              control={<Switch checked={settings.compactMode} onCheckedChange={(v) => updateSetting('compactMode', v)} />}
            />
            <Separator />
            <SettingRow
              label="Chart animations"
              description="Animate chart transitions and updates"
              control={<Switch checked={settings.chartAnimations} onCheckedChange={(v) => updateSetting('chartAnimations', v)} />}
            />
          </SettingsSection>
        </div>

        <div className="space-y-6">
          <SettingsSection title="Notifications" description="Alert and notification preferences" icon={Bell}>
            <SettingRow
              label="Push notifications"
              description="Receive desktop notifications for alerts"
              control={<Switch checked={settings.notifications} onCheckedChange={(v) => updateSetting('notifications', v)} />}
            />
            <Separator />
            <SettingRow
              label="Alert sounds"
              description="Play sound when critical alerts fire"
              control={<Switch checked={settings.soundEnabled} onCheckedChange={(v) => updateSetting('soundEnabled', v)} />}
            />
            <Separator />
            <SettingRow
          ...
          <SettingsSection title="About" description="System information and version" icon={Server}>
            <div className="space-y-2 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium text-foreground">2.4.1</span>
              </div>
              <Separator />
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Build</span>
                <span className="font-mono text-xs text-muted-foreground">2024.12.01-1337</span>
              </div>
              <Separator />
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Node</span>
                <span className="font-mono text-xs text-foreground">v20.11.0</span>
              </div>
              <Separator />
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Agent</span>
                <span className="font-mono text-xs text-foreground">v3.2.1</span>
              </div>
              <Separator />
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">License</span>
                <Badge variant="outline" className="text-[10px] text-muted-foreground">MIT</Badge>
              </div>
            </div>
          </SettingsSection>
        </div>
