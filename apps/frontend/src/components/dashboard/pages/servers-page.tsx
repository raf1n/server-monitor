...
const STATUS_STYLES: Record<ServerStatus, { label: string; dot: string; bg: string; border: string }> = {
  ...
  offline: { label: "Offline", dot: "bg-destructive", bg: "bg-destructive/10", border: "border-destructive/30" },
  degraded: { label: "Degraded", dot: "bg-warning", bg: "bg-warning/10", border: "border-warning/30" },
};

function ServerCard({
  server, isSelected, onSelect, showSensitiveData,
}: {
  server: ServerInfo; isSelected: boolean; onSelect: () => void; showSensitiveData: boolean;
}) {
  const style = STATUS_STYLES[server.status];
  return (
    <button onClick={onSelect} className="w-full text-left">
      <Card className={cn("relative overflow-hidden border p-5 transition-all hover:shadow-md", isSelected ? "border-primary ring-1 ring-primary" : "border-border hover:border-muted-foreground/30")}>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <div className={cn("flex h-10 w-10 items-center justify-center rounded-lg", style.bg)}>
              <Monitor className={cn("h-5 w-5", server.status === "online" ? "text-success" : server.status === "offline" ? "text-destructive" : "text-warning")} />
            </div>
            <div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">Managing {servers.length} server{servers.length !== 1 ? "s" : ""} across your infrastructure</p>
      </div>
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Card className="flex items-center gap-3 border-border p-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-success/10"><Monitor className="h-5 w-5 text-success" /></div>
          <div><p className="text-2xl font-semibold text-foreground">{onlineCount}</p><p className="text-xs text-muted-foreground">Online</p></div>
        </Card>
        <Card className="flex items-center gap-3 border-border p-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-warning/10"><Monitor className="h-5 w-5 text-warning" /></div>
          <div><p className="text-2xl font-semibold text-foreground">{degradedCount}</p><p className="text-xs text-muted-foreground">Degraded</p></div>
        </Card>
        <Card className="flex items-center gap-3 border-border p-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-destructive/10"><Monitor className="h-5 w-5 text-destructive" /></div>
          <div><p className="text-2xl font-semibold text-foreground">{offlineCount}</p><p className="text-xs text-muted-foreground">Offline</p></div>
        </Card>
      </div>
