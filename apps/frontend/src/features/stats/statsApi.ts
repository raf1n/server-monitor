...
export const statsApi = apiSlice.injectEndpoints({
  endpoints: (build) => ({
    getStatsHistory: build.query<MetricPoint[], { serverId: string; range: string }>({
      query: ({ serverId, range }) => `/servers/${serverId}/metrics?range=${range}`,
      providesTags: (_result, _error, { serverId }) => [{ type: "Stats", id: serverId }],
    }),
...
export const { useGetStatsHistoryQuery, useGetLatestStatsQuery, useLazyGetStatsHistoryQuery } =
  statsApi;
